...
import { Play, RotateCcw, Github, Heart, Monitor, Tablet, Smartphone, ZoomIn, ZoomOut, Download, Upload, Palette, Eye, EyeOff, Sun, Moon, Settings } from 'lucide-react';
...
console.log('Welcome to HTML Editor!');`;
  ...
  return (
    <div className={`min-h-screen flex flex-col ${darkMode ? 'dark bg-gray-900' : 'bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50'}`}>
      {/* Header */}
      <header className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white/80 backdrop-blur-md border-slate-200/60'} border-b shadow-sm`}>
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className={`text-2xl font-semibold ${darkMode ? 'text-white' : 'text-slate-800'} tracking-tight`}>HTML Editor</h1>
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-slate-500'} mt-0.5`}>Create, edit, and preview HTML in real-time</p>
              </div>
            </div>
          ...
          <ResizablePanelGroup direction="horizontal" className="flex-1">
            ...
            <ResizablePanel defaultSize={50} minSize={30}>
              <div className="h-full flex flex-col gap-4">
                ...
                {/* Component Library */}
                <Card className="h-48 bg-white/90 backdrop-blur-sm border-slate-200/60 shadow-xl">
                  <ComponentLibrary onInsertComponent={handleCodeChange} />
                </Card>
              </div>
            </ResizablePanel>
          </ResizablePanelGroup>
      ...
      {/* Footer */}
      <footer className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white/80 backdrop-blur-md border-slate-200/60'} border-t mt-auto`}>
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <div className={`flex items-center space-x-6 text-sm ${darkMode ? 'text-gray-400' : 'text-slate-600'}`}>
            ...
            <div className="flex items-center space-x-4">
              <div className={`text-xs ${darkMode ? 'text-gray-500' : 'text-slate-500'}`}>
                ...
                <kbd className={`px-2 py-1 ${darkMode ? 'bg-gray-700' : 'bg-slate-100'} rounded text-xs`}>Ctrl+S</kbd>
                <span className="mx-1">to save</span>
              </div>
              <Button
                variant="ghost"
                size="sm"
                className={darkMode ? 'text-gray-400 hover:text-gray-200' : 'text-slate-500 hover:text-slate-700'}
          ...
          <div className={`mt-4 pt-4 border-t ${darkMode ? 'border-gray-700' : 'border-slate-200/60'}`}>
            <p className={`text-xs ${darkMode ? 'text-gray-500' : 'text-slate-500'} text-center`}>
